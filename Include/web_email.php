...
if ($Type=="fintpassword"){
  ...
  echo'<script language="javascript">alert("已发送到你的'.$postEmail.'邮箱！");location.href="'.$fhurl.'";</script>'; 
     ...
     $umm=test_input($_POST['umima']);
     $urzm=test_input($_POST['uyzm']);
     $fhurl=str_replace("SEMCMS_Remail.php","",$_POST['furl']);
 ...
 $msg_email=test_input($_POST['mail']);
 $msg_content=test_input($_POST['tent']);
 $yzm=$_POST['yzm'];
 $msg_tel=$_POST['tel'];
 $names=test_input($_POST['name']);
 $msg_pid=test_input($_POST['PID']);
 $msg_languageID=test_input($_POST['languageID']);
 $msg_time=date("Y-m-d h:i:s",time()+8*60*60);
 $msg_ip=test_input(getRealIp());
