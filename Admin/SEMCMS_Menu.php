...
<?php
 ...
 $row = mysql_fetch_array(mysql_query("SELECT * FROM sc_menu WHERE ID=".$_GET["ID"]));
...
<table width="98%" class="table" cellpadding="0" cellspacing="0">
  ...
  <tr>
   ...
   <?php 
   //
 $sql=mysql_query("select * from sc_menu where languageID=".$_GET["lgid"]."");     
 ...
 ?>
  <tr><td align="center"><input type="checkbox" name="AID[]" id="AID[]" value="<?php echo $row['ID'];?>" /></td><td align="center"><?php echo $js; ?></td>  <td> <?php echo $row['menu_name'] ?> </td><td><?php echo $row['menu_link'] ?></td><td><?php echo $row['menu_paixu'] ?></td> <td><?php echo $row['menu_time'];?> </td><td align="center">  <a href="?type=edit&lgid=<?php echo $_GET['lgid'] ?>&ID=<?php echo $row['ID']; ?>&page=<?php echo $page; ?>"><img src="SC_Page_Config/Image/icons/page_white_edit.png" align="absmiddle" >编辑</a></td></tr>
